...
interface ERC20
{
    ...
    //El msg.sender envia a recipient la cantidad de amount
    function transfer(address recipient, uint256 amount) external returns(bool);

    //Devuelve bool con el resultado de la transacción
    //Funcion previa al allowance, el msg.sender aprueba al spender gastar amount de tokens
...
contract ERC20Basic is ERC20
{
    ...
    string public constant name = "Insurance ADP";
    string public constant symbol = "IF";
    uint8 public constant decimals = 4;

    // Mapping que cuenta con los balances que tiene cada wallet
    mapping(address => uint) balances;

    // Mapping que relaciona Una wallet, a que wallets aprovado gastar, y que cantidad
    ...
    // Comprueba si la direccion delegate, tiene permitido gastar tokens del owner, sino devuelve 0
    function allowance(address owner, address delegate) public override view returns(uint256){
        return allowed[owner][delegate];
    }

    function transfer(address recipient, uint256 _numTokens) public override returns(bool){
        require(_numTokens <= balances[msg.sender],'Yo need more IF');
